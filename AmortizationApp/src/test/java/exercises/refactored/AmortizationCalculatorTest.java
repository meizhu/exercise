...
/**
 * Tests for the refactored AmortizationCalculator, with test data from the original code output.
 */
public class AmortizationCalculatorTest {
	...
	@Test
    public void montlyDetailsResult_Match_Original() {
        ...
        for (int i = 0; i < payments.size(); ++i) {
        	...
        	assertThat("payment number", (double)payment.getPaymentNumber(), is(MONTHLY_RESULTS[i][0]));
        	assertThat("payment amount", payment.getPaymentAmount()/100D, is(MONTHLY_RESULTS[i][1]));
        	assertThat("payment interest", payment.getInterestPaying()/100D, is(MONTHLY_RESULTS[i][2]));
        	assertThat("current balance", payment.getRemaining()/100D, is(MONTHLY_RESULTS[i][3]));
        	assertThat("total payment", payment.getTotalPayments()/100D, is(MONTHLY_RESULTS[i][4]));
        	assertThat("total interest paid", payment.getTotalInterestPaid()/100D, is(MONTHLY_RESULTS[i][5]));
        }
	}
	   ...
	   * The test data is generated by running and reformatting the original code's output with the same loan input.
	   * This is used to ensure that the refactored code still outputs the same schedule table.
	   * 
